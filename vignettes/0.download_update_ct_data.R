  ...
  do({
    ...
    assign(
      ...
      load_some_ct_bulks_rdata(
        ...
        hsCodes = wtoCommodities
      )
    )
    save(list = .$destNewFile, file = file.path(toFolder, .$destNewFile))
    rm(list = .$destNewFile)
    gc()
    tibble()
  })
...
readr::write_rds(wto_an, path = file.path(toFolder, "AllInOne"), compress = "gz")
rm(wto_an)
gc()
...
readr::write_rds(wto_mn, path = file.path(toFolder, "AllInOne"), compress = "gz")
rm(wto_mn)
gc()
